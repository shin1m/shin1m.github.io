...
(function() {
  ...
  Match = (function(superClass) {
    var points0, points1, state_close, state_replay;
    ...
    state_replay = Stage.prototype.State(function() {
      var record;
      if (this.duration <= 0) {
        this.player0.resume();
        this.player1.resume();
      ...
      this.ball.replay(record.ball);
      this.mark.replay(record.mark);
      this.player0.replay(record.player0);
      this.player1.replay(record.player1);
      this.records.push(record);
      this.camera0.position.set((this.ball.position.x + this.ball.hitter.root_position().x) * 0.5, 4.0, (this.ball.position.z + 40.0 * this.ball.hitter.opponent.end) * 0.5);
      this.camera0.lookAt(new THREE.Vector3(0.0, -6.0, -40.0 * this.ball.hitter.opponent.end).add(this.camera0.position));
      this.camera1.position.set((this.ball.position.x + this.ball.hitter.opponent.root_position().x) * 0.5, 4.0, (this.ball.position.z + 40.0 * this.ball.hitter.opponent.end) * 0.5);
      return this.camera1.lookAt(new THREE.Vector3(0.0, -6.0, -40.0 * this.ball.hitter.opponent.end).add(this.camera1.position));
    }, {
      13: function() {
        this.player0.resume();
        this.player1.resume();
    ...
    Match.prototype.transit_replay = function() {
      this.player0.suspend();
      this.player1.suspend();
      this.state = state_replay;
      return this.duration = this.records.length * 2;
    };

    Match.prototype.reset_cameras = function() {
    ...
    Match.prototype.reset = function() {
      ...
      return this.reset_cameras();
    };

    Match.prototype.step_things = function() {
      var record;
      Match.__super__.step_things.call(this);
      record = this.records.shift();
      this.ball.record(record.ball);
      this.mark.record(record.mark);
      this.player0.record(record.player0);
      this.player1.record(record.player1);
    ...
    Match.prototype.initialize = function(controller0, player0, controller1, player1, container, next) {
      return Match.__super__.initialize.call(this, controller0, player0, controller1, player1, (function(_this) {
        return function() {
          ...
          _this.records = (function() {
            var j, results;
            results = [];
            for (i = j = 0; j < 150; i = ++j) {
              results.push({
                ball: this.ball.create_record(),
                mark: this.mark.create_record(),
                player0: this.player0.create_record(),
                player1: this.player1.create_record()
              });
            }
            return results;
          }).call(_this);
  ...
  showMenu = function() {
    var j, k, len, len1, menu, ref, ref1, screen, slider;
